...
int io300_readc(struct io300_file* f) { return fgetc(f->f); }

int io300_writec(struct io300_file* f, int ch) { return fputc(ch, f->f); }
...
int io300_flush(struct io300_file* f) { return fflush(f->f); }
