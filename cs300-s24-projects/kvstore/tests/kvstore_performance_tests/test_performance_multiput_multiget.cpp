  ...
  */
  ...
  auto query_all = [&]() {
    for (size_t i = 0; i < N_THREADS; i++) {
      ...
      ASSERT(multiget_range(*single_threaded_store, keys, values, 0,
                            N_KEYS_PER_THREAD, N_KEYS_PER_THREAD));
    }
  };

  // Define helper that threads use to MultiGet the keys in the multi-threaded
  // store. Note that since each entry in buckets_to_keys touches a single
  // bucket (that's unique from the other buckets in buckets_to_keys), we are
  // guaranteed that if you have implemented bucket-based locking correctly,
  ...
  auto query_bucket = [&](size_t i) {
    ...
    ASSERT(multiget_range(*multi_threaded_store, keys, values, 0,
                          N_KEYS_PER_THREAD, N_KEYS_PER_THREAD));
  };
  ...
  output_file << "single_thread_multiget," << single_threaded_time.count()
              << ","
              << to_throughput(single_threaded_time, N_THREADS,
                               N_KEYS_PER_THREAD)
              << "\n";

  // Time multi threaded multiget
  start = chrono::high_resolution_clock::now();
  {
    vector<thread> threads;
    for (size_t i = 0; i < N_THREADS; i++) {
  ...
  output_file << "multi_thread_multiget," << multi_threaded_time.count() << ","
              << to_throughput(multi_threaded_time, N_THREADS,
                               N_KEYS_PER_THREAD)
              << "\n";
